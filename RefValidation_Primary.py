...
#logic to find the md5sum of files at the master node location
    ...
    for pod in podList:
        if pod.find('rsyncd')== -1:
            ...
            if pod.find('chf-76489fbfb')== -1:
                status,md5Sum = subprocess.getstatusoutput('/usr/local/bin/kubectl -n {0} exec -it {1} -c {2} -- bash -c "md5sum {3}" 2>/dev/null'.format(provisioining,pod,container,chfLoc))
            else:
                status,md5Sum = subprocess.getstatusoutput('/usr/local/bin/kubectl -n {0} exec -it {1} -c {2} -- bash -c "md5sum {3}" 2>/dev/null'.format(ecs,pod,chf,chfLoc))
        else:
            status, md5Sum = subprocess.getstatusoutput(
                     '/usr/local/bin/kubectl -n {0} exec -it {1} -- bash -c "md5sum {2}" 2>/dev/null'.format('configuration',pod,rsyncLoc))
        if status == 0:
    ...
    if status ==0:
        ...
        with open(yestFile, 'r') as f:
            ...
            for lines in csv_reader:
                if lines[0] == filename:
                    yes_count = lines[2]
                elif lines[0].startswith("OC") and lines[0].endswith(".xml") and filename.startswith("OC") and filename.endswith(".xml"):
                    yes_count = lines[2]
...
#logic to chcek ref files in Ref Pod

def refPodfiles(key,filename):
    status,refpod = subprocess.getstatusoutput(
        "/usr/local/bin/kubectl get pod -n ref | grep -i ref-data-transformer | awk '{print $1}'")

    if status!=0:
        writer.write("<h3>Ref Pod NOT FOUND</h3>")
        exit(0)
    container = "ref-data-transformer"
    refFolders=['out','processed','in','error','duplicate']

    command = "find . -maxdepth 1 -type f -mtime -1"
    command2 = "awk -F \"/\" \'{print $2}\'"
    row_list=[]
    row_list.append(key)
    isColor=False
    for val in refFolders:
        if val == 'out':
            status, res = subprocess.getstatusoutput(
                '/usr/local/bin/kubectl -n ref exec -it {0} -c {1} -- bash -c "cd mount/host/{2}/{3}/;{4}|{5}" 2>/dev/null'.format(
                    refpod, container, key, val, command, command2))
            res = res[2:]
            print(key)
            print("res of ", val, " is = ", res)
            if status == 0:
                if res == "":
                    #writer.write("<tr style=color:Red><td>{}</td><td>FILE NOT GENERATED</td>".format(key))
                    row_list.append('FILE NOT GENERATED')
                else:
                    #writer.write("<tr style=color:green><td>{}</td><td>FILE GENERATED</td>".format(key))
                    row_list.append('FILE GENERATED')
            else:
                #writer.write("<tr style=color:red><td>{}</td><td>ERROR OCCURRED</td>".format(key))
                row_list.append('ERROR OCCURRED')
                isColor = True

        else:
            status, res = subprocess.getstatusoutput(
                '/usr/local/bin/kubectl -n ref exec -it {0} -c {1} -- bash -c "cd mount/host/{2}/{3}/;{4}|wc -l" 2>/dev/null'.format(
                    refpod, container, key, val, command))
            print("res of ", val, " is = ", res)
            if status == 0:
                if res == '0' or res == 0 :
                    #writer.write("<td>{}</td>".format(res))
                    row_list.append(res)
                else:
                    row_list.append(res)
                    if val != 'processed':
                ...
                #writer.write("<td>Error Occured</td>\n")
                row_list.append('Error Occured')
                isColor = True
    writer.write(con.list2html(row_list, False, isColor=isColor))


##########################################################################################################
def checkSpace():
    print("Inside CheckSpace")
    nameSpace = "ref"
    podName = "ref-data-transformer-0"
    containerName = "ref-data-transformer"
    status, res = subprocess.getstatusoutput('/usr/local/bin/kubectl -n {0} exec -it {1} -c {2} -- bash -c "df -kh|grep -E \'/dev/rbd.*/mount/host\'" 2>/dev/null'.format(nameSpace, podName, containerName))
    res = res.split(" ")
    print(res)
    writer.write("<table border=\"1\">\n")
    writer.write("<tr><th colspan=\"2\">Status Of File System in Ref Pod</th></tr>\n")
    writer.write("<tr><td>Used Percentage</td><td>Mounted On</td></tr>\n".format(res[-2], res[-1]))
    writer.write("<tr><td>{0}</td><td>{1}</td></tr>\n".format(res[-2],res[-1]))
    writer.write("</table>\n")
    print("End of CheckSpace")

#--------------------------------Logic to check File System Utilization in ref pod-----------
...
#-------------------------Driver Code for MasterNode md5sum-----------------------

'''
refDict = {"TLG_To_Roaming_Zone": ["LocationZoneTable.csv","/home/nccloud/CHF/ECS8.6.0/config-data/chf/product-config/rzd/"],
           "SGW_IP_To_PLMN": ["SgwIpToPlmnTable.csv","/home/nccloud/CHF/ECS8.6.0/config-data/chf/product-config/attc-ref-data/"],
           "RMS_To_Serving_SID": ["PlmnServingSidTable.csv","/home/nccloud/CHF/ECS8.6.0/config-data/chf/product-config/attc-ref-data/"],
           "Market_To_Timezone": ["MarketToTimeZoneTable.csv","/home/nccloud/CHF/ECS8.6.0/config-data/cog/product-config/attc-ref-data/"],
           "BRMS_To_TimeNotification": ["OfferIdToTimeBasedTimers.csv","/home/nccloud/CHF/ECS8.6.0/config-data/cog/product-config/attc-ref-data/"],
           "BRMS_To_OC": ["File not generated","/home/nccloud/CHF/ECS8.6.0/config-data/ocrt/product-config/offercatalog/config/"]}
'''
...
print("Master Node md5sum = ",masterPathmdsum,"\n")
...
for key in podLocationDict.keys():
    if key in ["TLG_To_Roaming_Zone", "SGW_IP_To_PLMN", "RMS_To_Serving_SID"]:
        ...
        #getPodmd5sumBRMS((podList, key, podLocationDict[key]["fileName"], podLocationDict[key]["Rsynd"],podLocationDict[key]["CHF"],'provisioning','custom-provisioning-gateway')
        ...
        #status, BRMS_To_OC_filename = subprocess.getstatusoutput('cd /home/nccloud/CHF/ECS8.6.0/config-data/ocrt/product-config/offercatalog/config/; find . -maxdepth 1 -type f -mtime -1')
        if status == 0:
...
#----------------------------------------------logic for ref files in Ref Pod------------------------------

writer.write("<br><br>")
writer.write("<table border=\"1\">\n")
writer.write("<tr><th colspan=\"6\">Status Of Ref files in Ref Pod</th></tr>\n")
writer.write("<tr><td>Market Name</td><td>Status(out directory)</td><td>File Count (processed directory)</td><td>File Count (in directory)</td><td>File Count (error directory)</td><td>File Count (duplicate directory)</td></tr>\n")

for key in refDict.keys():
    refPodfiles(key,refDict[key][0])
